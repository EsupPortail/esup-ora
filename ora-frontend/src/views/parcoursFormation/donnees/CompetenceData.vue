...
watch(
  () => props.data,
  (newGlobalData) => {
    console.log('comp data changaed')
    console.log(newGlobalData.globalData)
    let tmpRes = []
    if (gData.value.length === 0 && newGlobalData.globalData !== undefined) {
      tmpRes = newGlobalData.globalData
      console.log('heeeeeeeeeeelo')
      console.log(tmpRes)
      tmpRes.forEach((c) => {
        c.bccs = []
        console.log(c)
        c.connectedBccs.forEach(async (b) => {
          await bccStore.fetchBCCById(b).then((data) => {
            console.log(data)
            c.bccs.push({
              id: b,
              lib: data.libelle
