...
if grep -q 'ERROR' /var/log/shibboleth/shibd.log; then
    ...
    # [PATH_TO_SCRIPT]/back-to-yesterday-metadata.sh monappli.com && echo "Tout est OK. La mise à jour des metadata shibboleth est un succès." || echo "Erreur dans la mise à jour des metadata shibboleth. Rechargement des données de la veille." && /usr/bin/supervisorctl restart shibd
    exit 1
else
